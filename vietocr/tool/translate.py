...
#            output = model.transformer(src, tgt_inp, tgt_key_padding_mask=None)
        ...
        valid_char_probs_sum = (char_probs > 0).sum(-1)
        char_probs = np.sum(char_probs, axis=-1) / np.maximum(valid_char_probs_sum, 1) #sử dụng np.maximum để tránh phép chia cho 0.
...
def process_image(image, image_height, image_min_width, image_max_width):
    ...
    # Thử dùng mà không cần chỉ định ANTIALIAS hay LANCZOS
...
def process_input(image, image_height, image_min_width, image_max_width, device='cpu'):
    img = process_image(image, image_height, image_min_width, image_max_width)
    img = img[np.newaxis, ...]
    img = torch.FloatTensor(img).to(device)  # Ensure it moves to the proper device
    return img

def predict(filename, config, model=None, vocab=None):
    img = Image.open(filename)
    img = process_input(img, config['dataset']['image_height'], config['dataset']['image_min_width'], config['dataset']['image_max_width'], config['device'])
